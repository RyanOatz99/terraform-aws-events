...
def processRecords(records):
    for r in records:
        ...
        if (
            (data["detail-type"] == "CodeCommit Repository State Change")
            or (data["detail-type"] == "CodeCommit Comment on Commit")
            or (data["detail-type"] == "CodeCommit Comment on Pull Request")
        ):
            st = "aws:firehose:cloudwatchevents"
            other_findings += 1

        if (data["detail-type"] == "CodePipeline Stage Execution State Change") or (
            data["detail-type"] == "CodePipeline Action Execution State Change"
        ):
            st = "aws:firehose:cloudwatchevents"
            other_findings += 1

        if (data["detail-type"] == "CodeBuild Build State Change") or (
            data["detail-type"] == "CodeBuild Build Phase Change"
        ):
            st = "aws:firehose:cloudwatchevents"
            other_findings += 1

        if (
            data["detail-type"] == "CodeDeploy Deployment State-change Notification"
        ) or (data["detail-type"] == "CodeDeploy Instance State-change Notification"):
            st = "aws:firehose:cloudwatchevents"
            other_findings += 1

        if (
            (data["detail-type"] == "Config Rules Compliance Change")
            or (data["detail-type"] == "Config Rules Re-evaluation Status")
            or (data["detail-type"] == "Config Configuration Snapshot Delivery Status")
            or (data["detail-type"] == "Config Configuration History Delivery Status")
        ):
            st = "aws:firehose:cloudwatchevents"
            other_findings += 1
        ...
        if (
            (data["detail-type"] == "EC2 Instance State-change Notification")
            or (data["detail-type"] == "EBS Volume Notification")
            or (data["detail-type"] == "EBS Snapshot Notification")
            or (data["detail-type"] == "EC2 Spot Instance Interruption Warning")
        ):
            st = "aws:firehose:cloudwatchevents"
            other_findings += 1

        if (data["detail-type"] == "ECS Task State Change") or (
            data["detail-type"] == "ECS Container Instance State Change"
        ):
            st = "aws:firehose:cloudwatchevents"
            other_findings += 1

        if (
            (data["detail-type"] == "EMR Auto Scaling Policy State Change")
            or (data["detail-type"] == "EMR Step Status Change")
            or (data["detail-type"] == "EMR Cluster State Change")
            or (data["detail-type"] == "EMR Instance Group State Change")
            or (data["detail-type"] == "EMR Instance Fleet State Change")
            or (data["detail-type"] == "EMR Instance Group Status Notification")
        ):
            st = "aws:firehose:cloudwatchevents"
            other_findings += 1
        ...
        if (
            (data["detail-type"] == "Glue Job State Change")
            or (data["detail-type"] == "Glue Crawler State Change")
            or (data["detail-type"] == "Glue Job Run Status")
        ):
            st = "aws:firehose:cloudwatchevents"
            other_findings += 1
        ...
        if (
            (data["detail-type"] == "KMS Imported Key Material Expiration")
            or (data["detail-type"] == "KMS CMK Rotation")
            or (data["detail-type"] == "KMS CMK Deletion")
        ):
            st = "aws:firehose:cloudwatchevents"
            other_findings += 1
        ...
        if (data["detail-type"] == "MediaStore Object State Change") or (
            data["detail-type"] == "MediaStore Container State Change"
        ):
            st = "aws:firehose:cloudwatchevents"
            other_findings += 1
        ...
        if (data["detail-type"] == "MediaLive Channel State Change") or (
            data["detail-type"] == "MediaLive Channel Alert"
        ):
            st = "aws:firehose:cloudwatchevents"
            other_findings += 1

        if (
            (data["detail-type"] == "OpsWorks Instance State Change")
            or (data["detail-type"] == "OpsWorks Command State Change")
            or (data["detail-type"] == "OpsWorks Deployment State Change")
            or (data["detail-type"] == "OpsWorks Alert")
        ):
            st = "aws:firehose:cloudwatchevents"
            other_findings += 1
        ...
        if (
            (data["detail-type"] == "EC2 State Manager Association State Change")
            or (
                data["detail-type"]
                == "EC2 State Manager Instance Association State Change"
            )
            or (data["detail-type"] == "EC2 Command Status-change Notification")
            or (
                data["detail-type"]
                == "EC2 Command Invocation Status-change Notification"
            )
            or (data["detail-type"] == "Maintenance Window State-change Notification")
            or (
                data["detail-type"]
            ...
            or (data["detail-type"] == "EC2 Automation Step Status-change Notification")
            or (
                data["detail-type"]
                == "EC2 Automation Execution Status-change Notification"
            )
            or (data["detail-type"] == "Parameter Store Change")
            or (data["detail-type"] == "Configuration Compliance State Change")
            or (data["detail-type"] == "Inventory Resource State Change")
        ):
            st = "aws:firehose:cloudwatchevents"
            other_findings += 1

        if (data["detail-type"] == "Storage Gateway File Upload Event") or (
            data["detail-type"] == "Storage Gateway Refresh Cache Event"
        ):
            st = "aws:firehose:cloudwatchevents"
            other_findings += 1
        ...
        return_event["event"] = json.dumps(data)
